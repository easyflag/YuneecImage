...
public class ByteUtils {
    ...
    public static String byteArrayToHexString(byte[] byteArray, int start, int end, int length) {
        if (byteArray == null) {
            return "Length invalid";
        }
        StringBuffer outBuffer = new StringBuffer();
        for (int i = 0; i < end - start; ++i) {
            outBuffer.append(String.format("%02x ", byteArray[start + i]));
        }
        outBuffer.append("\n");
        return outBuffer.toString();
    }
    ...
    public static String byteArrayToHexString(byte[] byteArray) {
        ...
        StringBuffer outBuffer = new StringBuffer();
        for (int i = 0; i < length; ++i) {
            outBuffer.append(String.format("%02x ", byteArray[i]));
        }
        outBuffer.append("\n");
        return outBuffer.toString();
    }
    ...
    /**
     * get effect string value from the end of bytes ,trim 0 value,For example 58,102,25,0,0,0,36,790,0,0,0  ---> 58,102,25,0,0,0,36,79
     * @param datas byte[]
     * @return string
     *
     */
    public static String getByteStringFromEnd(byte[] datas) {
        int subIndex = 0;
        ...
        subIndex = Math.min(datas.length, subIndex + 1);
        String dataString;
        try {
            dataString = new String(datas, "UTF-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            return null;
        }
        return dataString.substring(0, subIndex);
    }

    /**
     * get effect string value from the start of bytes,trim 0 value,For example 58,102,25,0,0,0,36,79,0,0,0 ---> 58,102,25
     *
     * @param datas byte[]
     * @return string
     */
    public static String getByteStringFromStart(byte[] datas) {
        int subIndex;
        for (subIndex = 0; subIndex < datas.length; subIndex++) {
            if (datas[subIndex] == 0) {
                break;
            }
        }
